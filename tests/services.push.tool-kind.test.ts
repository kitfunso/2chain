...
test('push with tool_kind=skill persists kind and skips evals', async () => {
  const r = await push(storage, embedder, 'agent-1', baseInput({
    name: 'pushed-skill',
    tool_kind: 'skill',
  }));
  assert.equal(r.ok, true);
  if (!r.ok) return;
  // Eval must NOT have run for skills.
  assert.equal(r.eval_ms, 0);
  assert.equal(r.total_count, 0);
  assert.equal(r.cases.length, 0);
  // Status flipped to active and reliability is at the discovery gate.
  assert.equal(r.status, 'active');
  assert.equal(r.reliability_score, 0.95);
...
test('push with tool_kind=subagent skips evals', async () => {
  const r = await push(storage, embedder, 'agent-1', baseInput({
...
test('push with tool_kind=prompt skips evals', async () => {
  const r = await push(storage, embedder, 'agent-1', baseInput({
    name: 'pushed-prompt',
    tool_kind: 'prompt',
