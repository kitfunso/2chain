...
6. **Not a replacement for evals frameworks.** 2chain runs its own minimal pass/fail evals at publish time to populate the reliability score. Sophisticated eval orchestration (LangSmith, Braintrust, OpenAI Evals) is a separate concern; we offer hooks to plug those in but don't reinvent them.
